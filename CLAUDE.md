...
- **A route whose query can outrun the schema wraps its body in
  ...
  columns — the list loads and the save fails. Bundles widen what that covers:
  membership is part of the word list now, so a missing `word_bundles` takes
  the whole vocabulary down with it and `/api/words` needs the guard on the
  read, the save and the membership edit alike. DELETE stays unguarded — it
  names only `words`, and cannot be reached before the list has loaded.
...
- **A request that answers after the account changed is dropped.** Both list
  ...
  the effect: `saveWord` and `useBundles` compare against `accountRef`, so a
  create that answers late cannot drop one account's bundle into the next
  one's list.
