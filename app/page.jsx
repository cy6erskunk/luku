...
import { SKIP_KEY, SERVER_KEY, hasApiKey, tokenize, sentenceOf, findExistingWord, savedWordEntry, responseError, inBundle, isDue, shuffled } from "./lib/utils.js";
  ...
  return (
    <div style={{ minHeight: "100vh", background: D, color: "#e8e0d5", fontFamily: "Georgia,serif" }} onClick={() => setPopup(null)}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        {/* Back to the scan screen, which is also how the reader reaches the
            bundle launcher — so this keeps the session's new-word triage. The
            stale translations it used to leave behind are handled where they
      ...
      {showWordList && (
        <WordList
          ...
          error={banner}
          onDismissError={loadError ? undefined : dismissAction}
        />
      )}
