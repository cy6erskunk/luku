...
describe("gradeWord", () => {
  ...
  it("guards the update on the schedule it read, so a duplicate cannot re-apply", async () => {
    const sql = fakeSql([[CARD], [CARD]]);
    await gradeWord(sql, "u1", 7, 5);

    expect(sql.calls[1].text).toContain("date_trunc('milliseconds', next_review_at) = ");
    // The guard value is the card's schedule as it stood before grading.
    expect(sql.calls[1].values).toContain(CARD.next_review_at);
  });

  it("compares at the millisecond resolution the driver can actually round-trip", async () => {
    // TIMESTAMPTZ keeps microseconds but the driver hands JS a Date, so the
    // guard can only ever carry milliseconds. Comparing the stored value
    // as-is matched zero rows for every word still on `DEFAULT NOW()` — its
    // first review silently failed and the card came back next session.
    const card = { ...CARD, next_review_at: new Date("2026-08-13T06:00:00.123456Z") };
    const sql = fakeSql([[card], [card]]);
    await gradeWord(sql, "u1", 7, 1);

    expect(sql.calls[1].text).toContain("date_trunc('milliseconds', next_review_at) = ");
    expect(sql.calls[1].values).toContain("2026-08-13T06:00:00.123Z");
  });

  it("uses a caller-supplied guard when given one", async () => {
    const sql = fakeSql([[CARD], [CARD]]);
    await gradeWord(sql, "u1", 7, 5, { expectedNextReviewAt: "2026-01-01T00:00:00.000Z" });

    expect(sql.calls[1].values).toContain("2026-01-01T00:00:00.000Z");
  });

  it("refuses to write when the caller's guard is not a timestamp at all", async () => {
    const sql = fakeSql([[CARD], [CARD]]);
    expect(await gradeWord(sql, "u1", 7, 5, { expectedNextReviewAt: "not a date" })).toBeNull();
    expect(sql.calls).toHaveLength(1);
  });

  it("returns null when the guard matches nothing, meaning someone else graded it", async () => {
    const sql = fakeSql([[CARD], []]);
    expect(await gradeWord(sql, "u1", 7, 5)).toBeNull();
  });
