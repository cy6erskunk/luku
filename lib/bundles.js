 ...
 *
 * Scopes the bundle as well as the word, so the invariant holds uniformly:
 * every membership write checks both sides in the same statement. Scoping the
 * word alone would already be safe — a caller can only reach rows hanging off
 * their own words, and no cross-owner row can exist because the insert checks
 * both — but "both sides, always" is a rule a reader can check at a glance,
 * and one that survives a row arriving by some other route.
 */
