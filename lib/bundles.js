 ...
 *
 * Asking twice — two tabs, a double tap, a retry — must not produce two
 * bundles, and there is no transaction to wrap a check in. The no-op DO UPDATE
 * is what makes it one statement: DO NOTHING would return no row the second
 * time. `bundles.name` on the right keeps the stored spelling, so re-adding
 * "kotimaa" does not rename the user's "Kotimaa".
 *
 * The conflict target infers `bundles_user_name`. A bare function call needs no
 * parentheses — the grammar requires them only for an expression that would
 * otherwise be ambiguous, such as an operator expression.
 */
 ...
 *
 * Scoping the word alone would already be safe — a caller can only reach rows
 * hanging off their own words. It scopes the bundle too so that "both sides,
 * always" holds for every membership write, which is a rule a reader can check
 * at a glance.
 */
export async function removeWordFromBundle(sql, userId, wordId, bundleId) {
  const rows = await sql`
    DELETE FROM word_bundles wb
    USING words w, bundles b
    WHERE wb.word_id = w.id AND wb.bundle_id = b.id
