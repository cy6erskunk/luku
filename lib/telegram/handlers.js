...
import { answerCallbackQuery, editMessageText, isUneditableError, sendMessage } from "./api.js";
import {
  ACTION_GRADE,
  ACTION_LATER,
  ACTION_REVIEW,
...
async function handleLinkCommand(sql, { chatId, telegramUserId, username, code }) {
  let link;
  try {
    link = await claimCodeAndLink(sql, { code, telegramUserId, chatId, username });
  } catch (e) {
    if (isUniqueViolation(e)) {
      // The statement rolled back as a unit, so the code is still unspent and
      // "try again" below is genuinely actionable.
      return sendMessage(
        chatId,
  ...
  const email = await accountEmail(sql, link.user_id);
  ...
  const confirmation = [`✅ Connected${email ? ` to ${escapeHtml(email)}` : ""}.`];
 ...
 */
async function sendNextCard(sql, chatId, link, { justGraded = false } = {}) {
  const userId = link.user_id;
  const word = await nextDueWord(sql, userId);
  if (!word) {
    // A finished session ends with the schedule, so "nothing else today" also
    // says when the bot will come back.
    const done = justGraded ? sessionComplete(link) : allCaughtUp(await countWords(sql, userId));
...
async function handleGrade(sql, { chatId, messageId, link, wordId, grade, sig, callbackId }) {
  ...
  // The same next_review_at the signature covers, so a retry that slips past
  // the check above still cannot apply the grade a second time.
  const updated = await gradeWord(sql, link.user_id, wordId, grade, {
    expectedNextReviewAt: word.next_review_at,
  });
  if (!updated) return answerCallbackQuery(callbackId, "This card has moved on.");

  const done = receipt(updated, grade);
  await editMessageText(chatId, messageId, done.text, done);
  await answerCallbackQuery(callbackId);

  return sendNextCard(sql, chatId, link, { justGraded: true });
}

/**
 * Ends the session in place: the card's own message becomes the summary, which
 * is what clears the word the user was looking at along with its buttons. A
 * toast alone left the card sitting there looking live.
 */
async function handleStop(sql, { chatId, messageId, link, callbackId, later = false }) {
  await answerCallbackQuery(callbackId);

  const due = await countDueWords(sql, link.user_id);
  const summary = sessionStopped(link, { due, later });

  try {
    await editMessageText(chatId, messageId, summary.text, summary);
  } catch (e) {
    // A card past its edit window is the one failure a fresh message fixes, and
    // a stop that reports nothing is the bug being fixed here. Everything else —
    // a blocked chat, a rate limit — would fail the same way twice, so it goes
    // to Sentry instead of costing a second call.
    if (!isUneditableError(e)) throw e;
    await sendMessage(chatId, summary.text, summary);
  }
}
...
async function handleCallback(query) {
  ...
  switch (parsed.action) {
    ...
    case ACTION_REVIEW:
      await answerCallbackQuery(query.id);
      return sendNextCard(sql, chatId, link);
    case ACTION_STOP:
      return handleStop(sql, common);
    case ACTION_LATER:
      return handleStop(sql, { ...common, later: true });
    default:
