...
describe("claimCodeAndLink", () => {
  ...
  it("tolerates a missing username", async () => {
    ...
    const [, telegramUserId, chatId, username] = sql.calls[0].values;
    expect(telegramUserId).toBe(42);
    expect(chatId).toBe(42);
    expect(username).toBeNull();
  });

  it("writes no column the bot does not read back", async () => {
    const sql = fakeSql([[{}]]);
    await claimCodeAndLink(sql, ARGS);

    expect(sql.calls[0].text).not.toContain("secret_hash");
    // Four values: the code hash, then the three the caller supplied.
    expect(sql.calls[0].values).toHaveLength(4);
  });
