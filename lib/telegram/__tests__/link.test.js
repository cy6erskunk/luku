...
describe("createLinkCode", () => {
  it("invalidates earlier unused codes before inserting", async () => {
    const sql = fakeSql([[], []]);
    await createLinkCode(sql, "u1");

    expect(sql.calls[0].text).toContain("DELETE FROM telegram_link_codes");
    expect(sql.calls[0].values).toEqual(["u1"]);
    expect(sql.calls[1].text).toContain("INSERT INTO telegram_link_codes");
  });
  ...
  it("stores only the hash, never the plaintext code", async () => {
    ...
    const [codeHash, userId, ttl] = sql.calls[1].values;
