...
const mocks = vi.hoisted(() => ({ sent: [], edited: [], answered: [], sql: null }));
...
describe("callback guards", () => {
  ...
  it("acknowledges the stop button without sending anything", async () => {
    mocks.sql = fakeSql([[LINK]]);
    await handleUpdate(callback("x"));

    expect(mocks.sent).toHaveLength(0);
    expect(mocks.answered[0].text).toMatch(/stopped/i);
  });
