...
// Everything that talks to Telegram is stubbed, but the error classifier is
// pure and is exactly what the stop fallback is being tested for.
vi.mock("../api.js", async (importOriginal) => ({
  isUneditableError: (await importOriginal()).isUneditableError,
...
describe("grading a card", () => {
  ...
  it("guards the write with the schedule the signature covers", async () => {
    ...
    expect(update.text).toContain("date_trunc('milliseconds', next_review_at) = ");
    expect(update.values).toContain(WORD.next_review_at);
  });

  it("grades a card the driver handed back as a Date, microseconds and all", async () => {
    // The shape production actually sees: TIMESTAMPTZ arrives as a Date, which
    // is what both the signature and the write guard have to be built from.
    const word = { ...WORD, next_review_at: new Date("2026-08-13T06:00:00.123456Z") };
    const graded = { ...word, interval_days: 1 };
    mocks.sql = fakeSql([[LINK], [word], [word], [graded], []]);

    await handleUpdate(callback(`g:1:3:${signCard(CHAT, word)}`));

    expect(mocks.answered[0].text).toBeUndefined();
    expect(lastEdit().text).toContain("Again");
    expect(lastEdit().text).toContain("tomorrow");
    const update = mocks.sql.calls.find((c) => c.text.includes("UPDATE words SET"));
    expect(update.values).toContain("2026-08-13T06:00:00.123Z");
  });
...
describe("callback guards", () => {
  ...
  it("sends the summary as a new message when the card is too old to edit", async () => {
    ...
    mocks.editError = Object.assign(new Error("Bad Request: message can't be edited"), { code: 400 });
    await handleUpdate(callback("x"));

    expect(lastSent().text).toMatch(/Review stopped/i);
  });

  it("does not retry a failure a fresh message would hit too", async () => {
    mocks.sql = fakeSql([[SCHEDULED_LINK], [{ count: 1 }]]);
    mocks.editError = Object.assign(new Error("Forbidden: bot was blocked by the user"), { code: 403 });

    await expect(handleUpdate(callback("x"))).rejects.toThrow(/blocked/);
    expect(mocks.sent).toHaveLength(0);
  });
