...
describe("tgCall", () => {
  ...
  it("recognizes the message being past editing, or gone", () => {
    expect(isUneditableError({ code: 400, message: "Bad Request: message can't be edited" })).toBe(true);
    expect(isUneditableError({ code: 400, message: "Bad Request: message to edit not found" })).toBe(true);
    expect(isUneditableError({ code: 400, message: "Bad Request: MESSAGE_ID_INVALID" })).toBe(true);
  });

  it("leaves failures a second call would not survive alone", () => {
    // Retrying these as a fresh sendMessage would fail identically, so they
    // have to keep surfacing.
    expect(isUneditableError({ code: 403, message: "Forbidden: bot was blocked by the user" })).toBe(false);
    expect(isUneditableError({ code: 429, message: "Too Many Requests: retry after 30" })).toBe(false);
    expect(isUneditableError({ code: 400, message: "Bad Request: can't parse entities" })).toBe(false);
    expect(isUneditableError(null)).toBe(false);
  });
