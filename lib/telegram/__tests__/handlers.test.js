...
describe("linking", () => {
  // One statement claims the code and creates the link, then the email is read
  // for the confirmation.
  const EMAIL = [{ email: "matti@example.com" }];

  it("binds the account when the code is valid", async () => {
    mocks.sql = fakeSql([[LINK], EMAIL]);
  ...
  it("states the reminder schedule in the confirmation", async () => {
    mocks.sql = fakeSql([[{ ...LINK, reminder_hour: 9, timezone: "Europe/Helsinki" }], EMAIL]);
    await handleUpdate(message("/start Sup3rC0d3"));
  ...
  it("escapes the email, which may legally contain HTML-significant characters", async () => {
    // The confirmation is parse_mode HTML; an unescaped & makes Telegram reject
    // it *after* the link exists, leaving the user linked and unconfirmed.
    mocks.sql = fakeSql([[LINK], [{ email: "a&b@example.com" }]]);
    await handleUpdate(message("/start Sup3rC0d3"));

    expect(lastSent().text).toContain("a&amp;b@example.com");
    expect(lastSent().text).not.toContain("a&b@example.com");
  });

  it("claims the code by hash, not plaintext, in a single statement", async () => {
    mocks.sql = fakeSql([[LINK], EMAIL]);
    await handleUpdate(message("/start Sup3rC0d3"));

    expect(mocks.sql.calls[0].values[0]).toBe(sha256("Sup3rC0d3"));
    expect(mocks.sql.calls[0].text).toContain("WITH claimed AS");
  });
  ...
  it("still confirms when the account email cannot be read", async () => {
    mocks.sql = fakeSql([[LINK], new Error("no users_sync")]);
    await handleUpdate(message("/start Sup3rC0d3"));
  ...
  it("leaves the code unspent when the link conflicts, so 'try again' works", async () => {
    const dup = Object.assign(new Error("duplicate key"), { code: "23505" });
    mocks.sql = fakeSql([dup]);

    await handleUpdate(message("/start Sup3rC0d3"));

    // The claim and the insert are one statement, so the failure rolled back
    // the consumption too — no separate consume was ever issued.
  ...
  it("stores the sender's telegram id and chat, not values from the message body", async () => {
    ...
    const [, telegramUserId, chatId, username] = mocks.sql.calls[0].values;
...
describe("reminder settings", () => {
  ...
  it("rejects an hour outside 0-23 without writing", async () => {
    ...
    for (const bad of ["24", "-1", "9.5", "nine", ""]) {
      ...
      // The finder alone would also come back empty if the write's SQL were
      // reworded, so pin the count too: link lookup and touchLink, nothing else.
      expect(mocks.sql.calls).toHaveLength(2);
