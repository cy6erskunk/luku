...
describe("sendReminders", () => {
  ...
  it("skips a user with nothing due without claiming them", async () => {
    ...
    // No claim: a later hour in the same day can still reach them. Counted as
    // well as matched — a substring that stops matching would read as "no
    // claim" too, and the selection is the only statement this may run.
  ...
  it("hands the claim back when Telegram definitively refused", async () => {
    // An error_code means Telegram answered and did not deliver, so retrying
    // later today cannot duplicate anything.
    ...
    // Selection, claim, release — and nothing else, so "no disable" holds
    // however that statement is worded.
    expect(sql.calls).toHaveLength(3);
    expect(sql.calls.some((c) => c.text.includes("reminders_enabled = FALSE"))).toBe(false);
  });

  it("keeps the claim when the failure is an ambiguous transport error", async () => {
    // No error_code: the request may have reached Telegram and been accepted
    // before the response died. A missed reminder beats a duplicate one.
    mocks.failWith = new TypeError("fetch failed");
    const sql = fakeSql([[row({ last_reminded_on: "2026-08-12" })], CLAIMED]);

    const result = await sendReminders(sql);

    expect(result).toMatchObject({ failed: 1, sent: 0 });
    // Only the selection and the claim — no release.
    expect(sql.calls).toHaveLength(2);
    expect(sql.calls.filter((c) => c.text.includes("SET last_reminded_on ="))).toHaveLength(1);
  });
