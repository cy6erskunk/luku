...
const NOW = new Date("2026-08-13T19:47:00Z"); // 22:47 in Helsinki
const SCHEDULED = {
...
describe("closing messages", () => {
  ...
  it("adds the reminder schedule when the link is known", () => {
    const text = sessionComplete(SCHEDULED, { now: NOW }).text;
...
describe("nextReminderDay", () => {
  it("points at today while the reminder hour is still ahead", () => {
    // 22:47 Helsinki with a 23:00 reminder — today's has not gone out yet.
    expect(nextReminderDay({ ...SCHEDULED, reminder_hour: 23 }, { now: NOW })).toBe("today");
  });

  it("calls the hour already past 'soon', because the hourly pass owes it", () => {
    expect(nextReminderDay(SCHEDULED, { now: NOW })).toBe("soon");
  });

  it("moves to tomorrow once today's reminder has been sent", () => {
    const link = { ...SCHEDULED, last_reminded_on: "2026-08-13" };
    expect(nextReminderDay(link, { now: NOW })).toBe("tomorrow");
  });

  it("compares against the user's local date, not the UTC one", () => {
    // The same instant is 2026-08-13 22:47 in Helsinki but 2026-08-14 07:47 in
    // Auckland, where yesterday's stamp is spent and 21:00 is still ahead.
    const auckland = { ...SCHEDULED, timezone: "Pacific/Auckland", last_reminded_on: "2026-08-13" };
    expect(nextReminderDay(auckland, { now: NOW })).toBe("today");
    expect(nextReminderDay({ ...auckland, last_reminded_on: "2026-08-14" }, { now: NOW })).toBe("tomorrow");
  });

  it("accepts a Date for last_reminded_on as well as a string", () => {
    const link = { ...SCHEDULED, last_reminded_on: new Date("2026-08-13T00:00:00Z") };
    expect(nextReminderDay(link, { now: NOW })).toBe("tomorrow");
  });

  it("returns null rather than throwing on an unusable timezone", () => {
    expect(nextReminderDay({ ...SCHEDULED, timezone: "Mars/Olympus" }, { now: NOW })).toBeNull();
  });
});

describe("reminderLine", () => {
  it("names the day and hour of the next reminder", () => {
    const line = reminderLine({ ...SCHEDULED, last_reminded_on: "2026-08-13" }, { due: 3, now: NOW });
    expect(line).toContain("tomorrow at 21:00");
    expect(line).toContain("Europe/Helsinki");
  });

  it("promises only the standing schedule when nothing is due", () => {
    // Reminders skip a user with an empty queue, so no specific day is honest here.
    const line = reminderLine(SCHEDULED, { due: 0, now: NOW });
    expect(line).toContain("21:00");
    expect(line).not.toContain("tomorrow");
  });

  it("says reminders are paused, and points at /resume", () => {
    const line = reminderLine({ ...SCHEDULED, reminders_enabled: false }, { due: 3, now: NOW });
    expect(line).toContain("paused");
    expect(line).toContain("/resume");
  });

  it("still states the hour when the timezone cannot be resolved", () => {
    const line = reminderLine({ ...SCHEDULED, timezone: "Mars/Olympus" }, { due: 3, now: NOW });
    expect(line).toContain("21:00");
  });
});

describe("sessionStopped", () => {
  it("says the review is over, how many are left, and when the reminder comes", () => {
    const text = sessionStopped({ ...SCHEDULED, last_reminded_on: "2026-08-13" }, { due: 4, now: NOW }).text;
    expect(text).toContain("Review stopped");
    expect(text).toContain("4 words are still due");
    expect(text).toContain("tomorrow at 21:00");
  });

  it("pluralizes a single remaining word", () => {
    expect(sessionStopped(SCHEDULED, { due: 1, now: NOW }).text).toContain("1 word is still due");
  });

  it("offers a way back into the session while words remain", () => {
    const markup = sessionStopped(SCHEDULED, { due: 2, now: NOW }).reply_markup;
    expect(markup.inline_keyboard[0][0].callback_data).toBe("r");
    expect(markup.inline_keyboard[0][0].text).toBe("Resume review");
  });

  it("drops the resume button when the queue is empty", () => {
    const stopped = sessionStopped(SCHEDULED, { due: 0, now: NOW });
    expect(stopped.text).toContain("Nothing else is due");
    expect(stopped.reply_markup).toBeUndefined();
  });

  it("words the reminder's Later button as a postponement, not a stop", () => {
    const stopped = sessionStopped(SCHEDULED, { due: 2, later: true, now: NOW });
    expect(stopped.text).not.toContain("Review stopped");
    expect(stopped.text).toContain("2 words are still due");
    expect(stopped.reply_markup.inline_keyboard[0][0].text).toBe("Start review");
  });

  it("carries no card text, so the word under review goes away with it", () => {
    expect(sessionStopped(SCHEDULED, { due: 2, now: NOW }).text).not.toContain("juosta");
  });
});
...
describe("dueSummary and reminder", () => {
  ...
  it("let a reminder be dismissed", () => {
    expect(reminder(3).reply_markup.inline_keyboard[0][1].callback_data).toBe("l");
  });
...
describe("message length", () => {
  ...
  it("bounds pos too, which /api/words stores without a length check", () => {
    expect(cardBack(CHAT, { ...MONSTER, pos: huge }).text.length).toBeLessThan(4096);
  });
